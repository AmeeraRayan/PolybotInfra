...
echo "                      'curl YOLO_PRIVATE_IP:8000/health' command to talk to the YOLO instance (which should return a 200 status code)."
echo "Command: bastion_connect.sh $PUBLIC_IP $POLYBOT_PRIVATE_IP curl $YOLO_PRIVATE_IP:8000/health"
echo -e "-----------------------------------------------------------------------------------------------------------------"

export KEY_PATH=$(pwd)/private_key
OUTPUT=$(bash bastion_connect.sh $PUBLIC_IP $POLYBOT_PRIVATE_IP curl --silent --output /dev/null --write-out '%{http_code}' $YOLO_PRIVATE_IP:8000/health)
